...
#define INPUT_HAS_STRIPPED_DATA     0x0040
#define INPUT_HAS_JUNK              0x0080
#define INPUT_HAS_ERRORS            0x0100
#define OUTPUT_NEEDS_NEW_FILE       0x1000
#define OUTPUT_NEEDS_NEW_IDAT       0x2000
#define OUTPUT_HAS_ERRORS           0x4000
...
opng_print_ratio(unsigned long num, unsigned long denom, int force_percent)
{
   ...
   if (force_percent)
   {
      usr_printf("%lu%02u%%", integer_part, fractional_part);
      return;
   }
   ...
   if (integer_part < 100)
   {
      usr_printf("%lu.%02ux", integer_part, fractional_part);
      return;
   }
...
opng_optimize_impl(const char *infile_name)
{
   ...
   {
      if (osys_path_mkbak(tmp_buf, sizeof(tmp_buf), infile_name_local) == NULL)
         bakfile_name = NULL;
   }
   ...
   Try
   {
      ...
      else
      {
         ...
         infile = osys_fopen_at(
            (new_outfile ? infile_name_local : bakfile_name), "rb",
            process.in_datastream_offset, SEEK_SET);
         if (infile == NULL)
         ...
         Catch (err_msg)
         {
            OPNG_ENSURE(err_msg != NULL, "Mysterious error in opng_copy_file");
         }
         fclose(infile);  /* finally */
         if (err_msg != NULL)
            Throw err_msg;  /* rethrow */
