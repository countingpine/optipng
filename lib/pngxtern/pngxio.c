 ...
 *   part of libpng-1.4.  The original code, retrofitted as a back-port,
 *   has limitations: it is thread-unsafe, and it only allows two png_ptr
...
pngxio_read_write(png_structp png_ptr, png_bytep data, png_size_t length)
{
   ...
   case PNGX_IO_CHUNK_HDR:
      ...
      if (io_state_op == PNGX_IO_READING)
      {
         ...
         png_memcpy(data, crt_chunk_hdr + *crt_chunk_hdr_len_ptr, length);
         *crt_chunk_hdr_len_ptr += length;
         if (*crt_chunk_hdr_len_ptr < 8)
            return;
         *crt_len_ptr = png_get_uint_32(crt_chunk_hdr);
         /* png_memcpy(png_ptr->chunk_name, crt_chunk_hdr + 4, 4); */
      }
      else  /* io_state_op == PNGX_IO_WRITING */
      {
         png_memcpy(crt_chunk_hdr + *crt_chunk_hdr_len_ptr, data, length);
         *crt_chunk_hdr_len_ptr += length;
         if (*crt_chunk_hdr_len_ptr < 8)
            return;
         *crt_len_ptr = png_get_uint_32(crt_chunk_hdr);
